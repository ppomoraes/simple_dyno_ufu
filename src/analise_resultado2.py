...
# 2. Fit the data to the exponential function
#    p0: Initial guess for the parameters (a, b, c).
#        Good initial guesses can help curve_fit converge faster and more reliably.
#        If you don't have a good idea, try some sensible defaults.
#        For 'a', it could be around the initial y-value.
#        For 'b', if data is decreasing, try negative; if increasing, try positive.
#        For 'c', it could be an asymptote if present, or near the minimum/maximum y-value.
initial_guess = [1.0, -0.5, 0.1] # Example initial guess
...
try:
    ...
    # 3. Plot the original data and the fitted curve
    plt.figure(figsize=(10, 6))
    plt.scatter(x_data, y_data, label='Original Data', s=20)
    plt.plot(x_data, exponential_func(x_data, a_fit, b_fit, c_fit), color='red',
             label=f'Fitted Exponential: $y = {a_fit:.2f} \cdot e^{{{b_fit:.2f}x}} + {c_fit:.2f}$')
    ...
    print(f"Error: {e}. Could not find optimal parameters. Try adjusting initial_guess or checking your data.")
except ValueError as e:
    print(f"Error: {e}. Check if your data is appropriate for an exponential fit.")
